...
- `Log` (`audit_logs` — model `AuditLog`/router `auditLog` เดิม, rename 24 ก.ค. 2026): append-only
  (**ห้ามมี update/delete**) — id (Int autoincrement ตั้งแต่ 24 ก.ค. 2026 — เดิม cuid), userId, action (tRPC path),
  targetId (text เสมอ), detail (Json), createdAt; ผู้เขียนมีแค่ middleware `auditMutation` ใน `trpc.ts`
  (ทุก mutation สำเร็จ) + login ใน `auth.ts` + click telemetry ผ่าน `log.track`
...
-- หรือ "ค้างจากวันก่อน" (เงื่อนไขเดียวกับ workPlan.todo):
OR (endDate < วันนี้ AND actEnd IS NULL)
```
...
กรณี `OR` ตัวที่ 2 ค้างจากวันก่อน เพิ่มเพื่อกันแผนที่จบเดือนก่อนและยังไม่กดจบ หายจาก "รายการทั้งเดือน"
ทันทีที่เปลี่ยนเดือน — โชว์ในทุกเดือน (สัมพันธ์กับ "วันนี้" ไม่ใช่เดือนที่ดู) แต่กระทบเฉพาะ "รายการทั้งเดือน"
ปฏิทิน grid ยังวาดแค่แผนที่ทับเดือนจริงๆ เพราะ `buildWeekBars` อ้าง overlap ของช่วงวันจริง
...
- `engineerProcedure`: role = ENGINEER — **ตั้งแต่ 24 ก.ค. 2026 เหลือใช้แค่** `ticket.accept` +
  `site.update` + `site.delete` (เดิม WorkPlan ทุก mutation อยู่หลังชั้นนี้ → เจ้าของเลิกกติกา "CEO view-only" แล้ว)
- **WorkPlan/`site.create` = `protectedProcedure` + ownership** (24 ก.ค. 2026): CEO/Engineer สร้าง+จัดการ
  งานของตัวเองได้เหมือนกัน กันข้ามคนด้วย `plan.userId === ctx.user.sub` ไม่ใช่ role (ดู AGENT.md ข้อ 6)
- ชั้น query: Engineer ถูกบังคับ filter `userId = ตัวเอง` เสมอ (ignore ค่าที่ client ส่งมา) / CEO เห็นทุกคน
  และ mutation ทุกตัวเช็ค ownership (`plan.userId === ctx.user.sub`) ก่อนแก้ (ด่านกันข้ามคนตัวจริง)
...
### 6. "งานล่าช้า" มี 2 concept ที่อยู่คู่กันโดยตั้งใจ

| | `planStatus()` | `planDelayKind()` |
|---|---|---|
| คำถามที่ตอบ | สถานะ **ณ วันนี้** | **จริงเบี่ยงจากแผน** ไหม |
| ค่า | NOT_STARTED / NOT_STARTED_OVERDUE / IN_PROGRESS / IN_PROGRESS_OVERDUE / COMPLETED | START_DUE / START_LATE / END_DUE / END_LATE / null |
| ใช้ที่ | ชิป status ทุกหน้าที่แสดงแผน (`STATUS_META`) | หน้า `/delays` ของ CEO + `DelayTag` |
| อยู่ที่ | `apps/web/src/lib/status.ts` | `apps/api/src/lib/overdue.ts` (+ mirror ที่ `status.ts`) |

**ทำไมไม่ยุบรวม:** แผนที่จบช้า 10 วันยังเป็น `COMPLETED` โดยชอบธรรม (งานเสร็จแล้วจริง) แต่ก็เป็น
`END_LATE` โดยชอบธรรมเช่นกัน (จริงเบี่ยงจากแผน) — ยุบรวมจะต้องเพิ่ม status ใหม่แล้วกระทบทุกหน้า
ที่ใช้ `STATUS_META` (dashboard / banner / summary / ปฏิทิน) ทางออกคือให้ `DelayTag`
(`apps/web/src/components/delay-tag.tsx`) ติดป้าย "เริ่มช้า/จบช้า N วัน" ข้างชิป status
เฉพาะเคสที่ชิปมองไม่เห็น → 2 หน้าเล่าเรื่องเดียวกันโดยไม่ต้องแก้นิยามเดิม
(ไม่ติดป้ายเคส `START_DUE`/`END_DUE` เพราะชิป `*_OVERDUE` แดงบอกอยู่แล้ว)

**`START_DUE` vs `END_DUE` แยกด้วย `actStart`** — `planDelayKind` ให้ผลตรงกับ `planStatus`:
แผนที่ยังไม่กดเริ่มเลยแต่เลย `startDate` = `START_DUE` (เลยกำหนด**เริ่ม** = ชิป `NOT_STARTED_OVERDUE`)
ถึงจะเลย `endDate` ไปแล้วก็ตาม ส่วน `END_DUE` (เลยกำหนด**จบ** = ชิป `IN_PROGRESS_OVERDUE`) ต้อง
เริ่มแล้ว (`actStart != null`) หน้า `/delays` จึงแสดงงานล่าช้า **ทุกใบ** ครบทั้ง 4 ประเภท
(หนักกว่าชนะ: `END_LATE` > `END_DUE` > `START_LATE` > `START_DUE`)

**กติกาเทียบ: ระดับ "วันไทย" เสมอ** — `actStart`/`actEnd` เป็น timestamp เต็ม แต่
`startDate`/`endDate` เป็น `@db.Date` (เที่ยงคืน UTC = **07:00 น. ไทย**) จึงต้อง `dateOnlyICT()`
ก่อนเทียบทุกครั้ง เคยพลาดตรงนี้มาแล้ว (24 ก.ค. 2026): เทียบ `getTime()` ดิบทำให้ทุกคนที่กดเริ่มงาน
หลัง 7 โมงเช้าของวันที่วางแผนไว้ถูกนับเป็น "เริ่มช้า" — หน้า /delays รายงาน false positive เกือบ 100%
เงื่อนไขต้องตรงกับที่ `workPlan.start/finish` ใช้บังคับ delay reason เป๊ะ (AGENT.md ข้อ 5)
ไม่งั้นจะมีแถวที่ระบบบอกว่า "ช้า" แต่ไม่เคยมีใครถูกขอเหตุผล

**mirror 2 ฝั่ง:** web import runtime code ข้าม app ไม่ได้ → `planDelayKind`/`planDelayDays`
ถูกก็อปไว้ทั้ง `apps/api/src/lib/overdue.ts` และ `apps/web/src/lib/status.ts`
(รูปแบบเดียวกับ `dateOnlyICT`) เทสทั้งสองฝั่งใช้ **ชุดเคสเดียวกัน** เพื่อล็อกว่าให้ผลตรงกัน —
แก้สูตรฝั่งเดียวแล้วอีกฝั่งจะแดง

**เหตุผลความล่าช้าอัปเดตได้ระหว่างค้าง** (`workPlan.explainDelay`) — เดิมเหตุผลถูกเก็บตอนกด
เริ่ม/จบเท่านั้น แผน `END_DUE` (เลยกำหนดจบแต่ยังไม่กดจบ) จึงไม่มีเหตุผลเลย ทั้งที่เป็นแถวที่ CEO
ต้องการรู้มากที่สุด mutation นี้เขียนลงคอลัมน์เดิม (ไม่เพิ่ม schema): `endDate` ผ่านไปแล้ว →
`delayEndReason` / ยังไม่ถึง → `delayStartReason` แผนที่จบงานแล้วล็อก (กติกาเดียวกับ `unstart`)
...
- `/dashboard` = 4 มุมมองของ WorkPlan table เดียว เรียงจากบนลงล่าง (scope ดู CONTEXT.md):
  ...
  ปุ่ม mutation gate ด้วย **ownership** (`plan.userId === myId`) ไม่ใช่ role (ตั้งแต่ 24 ก.ค. 2026):
  Engineer เห็นแต่แผนตัวเอง → เห็นปุ่มทุกแผน / CEO เห็นทุกแผนแต่เห็นปุ่ม (เพิ่ม/แก้/เริ่ม/จบ/ลบ) เฉพาะแผนที่ตัวเองสร้าง
  ปุ่ม "+ เพิ่มแผน" แสดงทุก role (แผนใหม่เป็นของคน login)
...
- มุมมอง 3 และ 4 ใช้ query แยก `workPlan.todo`
  ...
  **เหตุผลที่ยังแยก `todo` จาก `list`** (ทั้งที่ `list` ตอนนี้เห็นแผนค้างแล้ว):
  `list` ผูกกับเดือนที่เลือกในปฏิทิน (cache ขยับตาม `view`) ส่วน banner/สรุปอยู่นอกปฏิทิน
  จึงต้องใช้ query อิสระที่รวมแผนที่ทับ "วันนี้" เสมอ ไม่ว่าจะดูเดือนไหน
